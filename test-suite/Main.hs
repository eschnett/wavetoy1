...
specCell = parallel $ do
    describe "initCell" $ do
        it "is not all zero" $ property $ \t x ->
            let c = initCell (t, x) :: Cell Double
            in  u c /= 0 || rho c /= 0 || vx c /= 0
    describe "errorCell" $ do
        it "is zero when exact" $ property $ \t x ->
            let c = initCell (t, x) :: Cell Double
                e = errorCell (t, x) c
            in  e == Cell 0 0 0
        it "is not all zero when inexact" $ property $ \t x t' x' ->
            (t /= t' || x /= x')
                ==> let c = initCell (t, x) :: Cell Double
                        e = errorCell (t', x') c
                    in  e /= Cell 0 0 0
    describe "energyCell" $ do
        it "is zero for vacuum" $ property $ \u ->
            let c = Cell u 0 0 :: Cell Double
                e = energyCell c
            in  e == 0
        it "has the correct potential term" $ property $ \rho ->
            let c = Cell 0 rho 0 :: Cell Double
                e = energyCell c
            in  e == 0.5 * rho ^ 2
        it "has the correct kinetic term" $ property $ \vx ->
            let c = Cell 0 0 vx :: Cell Double
                e = energyCell c
            in  e == 0.5 * vx ^ 2
    describe "rhsCell" $ do
        it "is not all zero" $ property $ \t x t' x' ->
            let c  = initCell (t, x) :: Cell Double
                cx = initCell (t', x') :: Cell Double
                r  = rhsCell c cx
            in  u r /= 0 || rho r /= 0 || vx r /= 0
    describe "bcCell" $ do
        it "is possibly a reflection" $ property $ \t x ->
            let
...
specGrid = parallel $ do
    describe "skeletonGrid" $ do
        it "has the right extent" $ property $ \xmin xmax np ->
            (np > 3 && xmax > xmin)
                ==> let skel = skeletonGrid (xmin, xmax) np :: Grid Double ()
                    in  bnds skel == (xmin, xmax)
        it "has the right size" $ property $ \xmin xmax np ->
            (np > 3 && xmax > xmin)
                ==> let skel = skeletonGrid (xmin, xmax) np :: Grid Double ()
                    in  length (cells skel) == np
    describe "coordGrid" $ do
        it "is monotonic" $ property $ \xmin xmax np ->
            (np > 3 && xmax > xmin)
    ...
    describe "integralGrid" $ do
        it "is linear" $ property $ \alpha beta xmin xmax np ->
            (np > 3 && xmax > xmin)
                ==> let
                        skel   = skeletonGrid (xmin, xmax) np
                        coords = coordGrid skel :: Grid Double Double
                        int =
                    ...
                    in
                        approxEq scale int good_int
    describe "normGrid" $ do
        it "is positive" $ property $ \alpha xmin xmax np ->
            (np > 3 && xmax > xmin)
                ==> let skel   = skeletonGrid (xmin, xmax) np
                        coords = coordGrid skel :: Grid Double Double
                        ...
                        norm   = normGrid g
                    in  norm >= 0
        it "is linear" $ property $ \beta xmin xmax np ->
            (np > 3 && xmax > xmin)
                ==> let skel   = skeletonGrid (xmin, xmax) np
                        coords = coordGrid skel :: Grid Double Double
    ...
    describe "initGrid" $ do
        it "has the right time" $ property $ \t xmin xmax np ->
            (np > 3 && xmax > xmin)
                ==> let skel   = skeletonGrid (xmin, xmax) np
    ...
    describe "errorGrid" $ do
        it "is zero when exact" $ property $ \t xmin xmax np ->
            (np > 3 && xmax > xmin)
                ==> let skel   = skeletonGrid (xmin, xmax) np
                        coords = coordGrid skel
                        g      = initGrid t coords :: Grid Double (Cell Double)
                        err    = errorGrid g
                    in  all (all (~~ 0)) err
        it "is not all zero when inexact" $ property $ \t t' xmin xmax np ->
            (t /= t' && np > 3 && xmax > xmin)
                ==> let skel   = skeletonGrid (xmin, xmax) np
                        coords = coordGrid skel
                        g      = initGrid t coords :: Grid Double (Cell Double)
    ...
    describe "energyGrid" $ do
        it "is positive" $ property $ \t xmin xmax np ->
            (np > 3 && xmax > xmin)
                ==> let skel   = skeletonGrid (xmin, xmax) np
                        coords = coordGrid skel
                        g      = initGrid t coords :: Grid Double (Cell Double)
                    ...
                    in  etot > 0
    describe "rhsGrid" $ do
        it "does stuff" $ property $ \t xmin xmax np ->
            (np > 3 && xmax > xmin)
                ==> let skel   = skeletonGrid (xmin, xmax) np
                        coords = coordGrid skel
                        g      = initGrid t coords :: Grid Double (Cell Double)
                    ...
                    in  norm >= 0
    describe "rk2Grid" $ do
        it "does stuff" $ property $ \t xmin xmax np dt ->
            (np > 3 && xmax > xmin)
                ==> let skel   = skeletonGrid (xmin, xmax) np
                        coords = coordGrid skel
                        g      = initGrid t coords :: Grid Double (Cell Double)
