...
densitizeGrid g = g { cells = V.imap dens (cells g) }
  where
    dens i x | i == 0 || i == np - 1 = 0
    dens i x | i == 1 || i == np - 2 = 1 / 2 * x
    dens i x | otherwise             = x
    np = npGrid g
...
derivGrid g = g { cells = V.generate np deriv }
  where
    deriv i | i == 0      = diff (1 / dx) (cs ! 0) (cs ! 1)
    deriv i | i == np - 1 = diff (1 / dx) (cs ! (np - 2)) (cs ! (np - 1))
    deriv i | otherwise   = diff (1 / (2 * dx)) (cs ! (i - 1)) (cs ! (i + 1))
